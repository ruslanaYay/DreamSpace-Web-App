...
export const WishlistDetails = () => {
  ...
  const fetchWishlistData = async (page = 0) => {
    ...
    try {
      setLoading(true);
      // Запит на деталі вішліста + запит на бажання з пагінацією та сортуванням
      const [wishlistResponse, wishesResponse] = await Promise.all([
        fetch(`http://localhost:8085/api/wishlists/${id}`, { method: 'GET', headers }),
        fetch(`http://localhost:8085/api/wishlists/${id}/wishes?page=${page}&size=${itemsPerPage}&sort=id,desc`, { method: 'GET', headers })
      ]);

      if (!wishlistResponse.ok) throw new Error('Не вдалося завантажити дані вішліста');
      if (!wishesResponse.ok) throw new Error('Не вдалося завантажити бажання');
      ...
      // ІНТЕГРАЦІЯ API: Беремо дані з content
  ...
  return (
    <div className="container-fluid p-4 min-vh-100" style={{ backgroundColor: '#F3F8FE' }}>
      ...
      <div className="wishes-grid">
        {/* Картка створення (Завжди перша на будь-якій сторінці) */}
